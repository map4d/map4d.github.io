...
> **Map4dMap Flutter package** cung cấp Map4dMap Cổng 1 cửa ĐTQG SDK cho cả thiết bị Android và iOS, cho phép tùy chỉnh bản đồ với nội dung để hiển thị trên các thiết bị di động.  
...
Để sử dụng Map4dMap plugin, thêm `map4d_map_dtqg` vào mục **dependencies** trong file `pubspec.yaml` của Flutter project.

```yaml
dependencies:
  map4d_map_dtqg: ^0.1.0
```

Import `map4d_map_dtqg` vào project
...
* iOS: yêu cầu phiên bản iOS 12.0 trở lên
...
class _Map4dSampleState extends State<Map4dSample> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: _buildingsEnabledToggler,
        tooltip: '3D',
  ...
  void _buildingsEnabledToggler() async {
    setState(() {
